...
import com.together.view.ui.ChatViewGenerator;
...
public class SpeakingActivity extends
		...
		SpeakingPresenter.View, ChatViewGenerator.Callback {
	...
	public void clap(ItemChat item, int position) {
		getOps().clap(item, position);
	}

	@Override
	public void updateClap(int position, int clap) {
		ItemChat item = mChatAdapter.getItem(position);
		item.setClap(clap);
		
		mChatAdapter.updateItem(item, position);
	}
